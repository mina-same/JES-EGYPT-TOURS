...
export default function NewAdministratorPage() {
  ...
  const groupedPermissions = useMemo(() => {
    ...
    const entries = ALL_PERMISSIONS.map((value) => {
      ...
      const label = `${resourceRaw?.replace(/_/g, " ") || ""}: ${
        actionRaw || ""
      }`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isSuperAdmin) {
        setError("Only Super Admin can create administrators.");
        return;
      }
      ...
      setError(err?.response?.data?.error || err?.message || "Failed to create administrator");
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="grid gap-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>General information about administrator creation</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    <div className="text-sm text-muted-foreground">
                      After creation, you can edit the administrator from the list page.
                    </div>
                  </CardContent>
                </Card>
              </div>
