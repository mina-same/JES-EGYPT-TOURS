...
export default function SliderContentPage() {
  ...
  const savePromo = async () => {
    const payload: SliderUnderPromo = {
      text: promoForm.text.trim(),
      linkText: promoForm.linkText.trim(),
      link: promoForm.link.trim(),
      linkDirection: promoForm.linkDirection,
    };

    if (!payload.text || !payload.linkText || !payload.link) {
      toast({
  ...
  const clearPromo = async () => {
    try {
      ...
      setPromo(null);
      setPromoForm({ text: '', linkText: '', link: '', linkDirection: '_self' });
      toast({
        title: 'Cleared',
      ...
      if (response.success && response.data) {
        ...
        const filtered = !s
          ...
          : response.data.filter((item) =>
              item.title.toLowerCase().includes(s) || item.subtitle.toLowerCase().includes(s)
            );
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="font-semibold text-sm text-gray-900 truncate">
              {item.title}
              <span className="text-[#63ab45]">{item.titleSpan}</span>
              {item.titleEnd}
            </div>
            ...
            <div className="mt-1 flex flex-col gap-1 text-xs text-gray-500">
              ...
              {item.button?.text && <div>Button: {item.button.text}</div>}
            </div>
          </div>
        </div>
      ),
      ...
      <div className='rounded-xl border border-gray-200 bg-white p-4'>
        ...
        <div className='mt-4 grid grid-cols-1 gap-3 md:grid-cols-4'>
          <label className='block text-xs text-gray-600 md:col-span-2'>
            <div className='mb-1'>Promo Text</div>
            <input
              ...
              onChange={(e) => setPromoForm((p) => ({ ...p, text: e.target.value }))}
              disabled={promoLoading || promoSaving}
            />
          </label>

          <label className='block text-xs text-gray-600'>
            <div className='mb-1'>Link Text</div>
            <input
              className='w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-[#63ab45]'
              value={promoForm.linkText}
              onChange={(e) => setPromoForm((p) => ({ ...p, linkText: e.target.value }))}
              disabled={promoLoading || promoSaving}
            />
          </label>
          ...
          <label className='block text-xs text-gray-600 md:col-span-4'>
