...
export default function TourSubcategoryViewPage() {
  ...
  const fetchEntity = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await tourSubcategoryAPI.getById(id);
      if (res?.success && res.data) setEntity(res.data);
      else setError(res?.error || 'Tour subcategory not found');
    } catch (e: any) {
      setError(e?.response?.data?.error || e?.message || 'Failed to load the tour subcategory');
    } finally {
      setLoading(false);
    }
  }, [id]);
