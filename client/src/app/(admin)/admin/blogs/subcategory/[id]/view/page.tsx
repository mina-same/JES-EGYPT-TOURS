...
export default function BlogSubcategoryViewPage() {
  ...
  const fetchEntity = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await blogSubcategoryAPI.getById(id);
      if (res?.success && res.data) setEntity(res.data);
      else setError(res?.error || 'Blog subcategory not found');
    } catch (e: any) {
      setError(e?.response?.data?.error || e?.message || 'Failed to load the blog subcategory');
    } finally {
      setLoading(false);
    }
  }, [id]);
          ...
          {breadcrumbs.length > 0 && (
            <div><label className="text-[11px] uppercase font-bold text-gray-400">Breadcrumbs</label>
              <ul className="list-disc pl-5 text-[15px] text-gray-700 dark:text-gray-200 space-y-1 m-0">
                {breadcrumbs.map((b, i) => <li key={i}>{strictText(b.name, previewLocale) || '(no name)'} {b.url ? <span className="text-gray-400 text-xs">→ {b.url}</span> : null}</li>)}
              </ul>
            </div>
          )}
          {faqs.length > 0 && (
